...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        n_x = images_X.shape[0]
        n_y = images_Y.shape[0]

        # 1. Compute the discriminator losses on real images
        # D_X_loss = ...
        # D_Y_loss = ...
        D_X_loss = torch.nn.functional.mse_loss(D_X(images_X), torch.ones(n_x))
        D_Y_loss = torch.nn.functional.mse_loss(D_Y(images_Y), torch.ones(n_y))
        ...
        # D_X_loss = ...
        D_X_loss = torch.nn.functional.mse_loss(D_X(fake_X), torch.zeros(n_y))
        ...
        # D_Y_loss = ...
        D_Y_loss = torch.nn.functional.mse_loss(D_Y(fake_Y), torch.zeros(n_x))
        ...
        # g_loss = ...
        g_loss = torch.nn.functional.mse_loss(D_X(fake_X), torch.ones(n_y))

        if opts.use_cycle_consistency_loss:
            reconstructed_Y = G_XtoY(fake_X)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            # cycle_consistency_loss = ...
            cycle_consistency_loss = torch.nn.functional.mse_loss(reconstructed_Y, images_Y)
        ...
        # g_loss = ...
        g_loss = torch.nn.functional.mse_loss(D_Y(fake_Y), torch.ones(n_x))

        if opts.use_cycle_consistency_loss:
            reconstructed_X = G_YtoX(fake_Y)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            # cycle_consistency_loss = ...
            cycle_consistency_loss = torch.nn.functional.mse_loss(reconstructed_X, images_X)
            g_loss += cycle_consistency_loss
